...
local info = {
    ...
    version = "Aplha 0.1",
    author = "LUA MODS",
    -- Çàìåíè íà ñâîþ ïðÿìóþ ññûëêó íà GitHub (RAW)
...
local updateStatus = u8"Íàæìèòå äëÿ ïðîâåðêè"

-- Öâåòà (float ìàññèâû äëÿ mimgui)
        ...
        downloadUrlToFile(info.updateUrl, tempFile, function(id, status, p1, p2)
            if status == 6 then
                ...
                if f then
                    ...
                    if content == info.version then updateStatus = u8"Ó âàñ ïîñëåäíÿÿ âåðñèÿ"
                    else updateStatus = u8"Íàéäåíî îáíîâëåíèå: v" .. u8(content) end
                end
            elseif status == -1 then updateStatus = u8"Îøèáêà ñîåäèíåíèÿ" end
        end)
    end)
end
        ...
        sampRegisterChatCommand(command, function()
            lua_thread.create(function()
                for line in b.txt:gmatch("[^\r\n]+") do
                    if #line > 0 then
                        -- ÔÈÊÑ ÊÐÀÊÎÇßÁÐ: Ïåðåâîä èç UTF8 â CP1251
...
imgui.OnFrame(function() return isReady and winState[0] end, function()
    ...
    if imgui.Begin(u8(info.name), winState, imgui.WindowFlags.NoCollapse) then
        ...
        if imgui.Button(u8"ÁÈÍÄÛ", imgui.ImVec2(btnW, 35)) then tab = 1 end
        imgui.SameLine(); if imgui.Button(u8"ÍÀÑÒÐÎÉÊÈ", imgui.ImVec2(btnW, 35)) then tab = 2 end
        imgui.SameLine(); if imgui.Button(u8"ÎÁÍÎÂËÅÍÈß", imgui.ImVec2(btnW, 35)) then tab = 3 end
        imgui.SameLine(); if imgui.Button(u8"ÈÍÔÎ", imgui.ImVec2(btnW, 35)) then tab = 4 end
        imgui.Separator()

        if tab == 1 then
            imgui.InputText(u8"Êîìàíäà", addCmd, 64)
            imgui.InputTextMultiline(u8"Òåêñò", addTxt, 2048, imgui.ImVec2(-1, 80))
            if imgui.Button(u8"ÑÎÕÐÀÍÈÒÜ", imgui.ImVec2(-1, 30)) then
        ...
        elseif tab == 2 then
            ...
            if imgui.ColorEdit4(u8"Ôîí Îêíà", bgColor) then saveAll() end
            imgui.Text(u8"Öâåò êíîïîê:")
            if imgui.ColorEdit4(u8"Êíîïêè", btnColor) then saveAll() end

        elseif tab == 3 then
            imgui.Text(u8"Âåðñèÿ: " .. info.version)
            imgui.Text(u8"Ñòàòóñ: " .. updateStatus)
            if imgui.Button(u8"ÏÐÎÂÅÐÈÒÜ", imgui.ImVec2(-1, 40)) then checkUpdates() end

        elseif tab == 4 then
            imgui.Text(u8"Àâòîð: " .. info.author)
            imgui.Text(u8"Âåðñèÿ: " .. info.version)
